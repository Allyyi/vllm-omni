...
"""MiniCPM-O 4.5 TTS decoder: Llama-backbone AR model that generates audio codes."""
...
# Import MiniCPMTTSConfig from the original model via trust_remote_code
# The config is loaded by HuggingFace AutoConfig and passed in via hf_config
try:
    from configuration_minicpmo import MiniCPMTTSConfig
except ImportError:
    # Fallback: define a minimal config class if the original is unavailable
...
class MiniCPMTTSGenerationOutput(ModelOutput):
    ...
    Args:
        new_ids (torch.LongTensor): Newly generated audio code sequence,
            shape (batch_size, sequence_length, num_vq).
        audio_input_ids (torch.LongTensor): Updated input IDs including
            condition and generated audio codes,
            shape (batch_size, full_sequence_length, num_vq).
        past_key_values (Tuple[Tuple[torch.FloatTensor]]): Tuple containing
            pre-computed keys and values used for attention mechanism.
    ...
    past_input_ids: torch.LongTensor | None = None
    finished: bool = None
...
def make_streaming_chunk_mask_inference(
    ...
    # Create a complete attention mask for input embeds
    # [batch_size, seq_len], without considering audio mask
    # as audio is always at the end
...
class MiniCPMTTS(PreTrainedModel):
    ...
    def generate(
        ...
        generated_input_ids = new_tokens[:, 0:t, :]
    ...
    def generate_chunk(
        ...
        for t in range(max_new_token):
            ...
            if t == 0:
                ...
                # Generate the following audio tokens, it is applicable
                # to all other cases, including second and the following
                # calling of `generate`
        ...
        # The latest generated token is not in the range returned
        # this time. If it is an eos token, it is not returned.
        # If it is a normal token, it is not returned.
        generated_input_ids = new_tokens[:, 0:t, :]

        return generated_input_ids, past_key_values
    ...
    def interleaved_generate(
        ...
        **kwargs,
        ...
        |Text BOS|Spk embeds|Text-Hidden states Interleave (if applicable)|Audio BOS|
        where the last position is the audio BOS token.
        So, the first iteration in generation directly forward the model
        with inputs_embeds, and the last hidden states of the last position
        (Audio BOS) will be decoded to get the first audio token.
        """
...
# ---------------------------------------------------------------------------
# vLLM-Omni registry wrapper
# ---------------------------------------------------------------------------


class MiniCPMOTTSForConditionalGeneration(nn.Module):
    """
    Wrapper for MiniCPMTTS that conforms to the vLLM-Omni stage interface.

    This class is registered in the vLLM-Omni model registry and can be
    instantiated via ``init_vllm_registered_model``. It delegates to
    ``MiniCPMTTS`` for the actual TTS generation.
    """
    ...
    def __init__(self, *, vllm_config, prefix: str = ""):
        ...
        self.tts = MiniCPMTTS(config=tts_config, audio_tokenizer=None)
        self.config = config
    ...
    def forward(
        ...
        **kwargs,
        ...
        if inputs_embeds is None:
            raise ValueError("TTS stage requires inputs_embeds")
        ...
        return OmniOutput(
            ...
            multimodal_outputs={"audio_token_ids": outputs.new_ids},
        )
    ...
    def load_weights(self, weights):
        ...
        for name, tensor in weights:
            clean_name = name.removeprefix("tts.")
