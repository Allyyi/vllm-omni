...
class GenerateChunkOutput:
    ...
    input_last_hidden_states: torch.Tensor | None  # for tts use_speaker_embedding
    last_hidden_states: torch.Tensor | None  # for tts input feature (projector_semantic)
...
class ChunkPrefillChunkGenerate:
    ...
    def chunk_generate(
        ...
        Args:
            ...
            length_penalty: float, The length penalty for the model,
                default is 1.0. Higher value means more detailed
                generation.
        ...
        if len(generated_tokens) > 0:
            ...
            # special case: if last chunk and first predict is eos token,
            # return last token of previous chunk.
            # return a tensor with shape (1, 0)
            if finished:
...
class SpeculativeSnapshot:
    ...
    mel_processor_snapshot: dict | None = None

    # Audio encoder KV cache (requires cloning to ensure determinism after recovery)
    audio_past_key_values: tuple | None = None

    # timestamp (for debugging)
    timestamp: float = 0.0

    # debug field: for verifying correctness of recovery
    llm_cache_checksum: float | None = None  # LLM KV Cache first layer K sum
    audio_cache_checksum: float | None = None  # Audio KV Cache first layer K sum
    mel_buffer_checksum: float | None = None  # Mel buffer sum

    # RNG state (key: for ensuring determinism of dithering etc. after recovery)
    rng_state_cpu: torch.Tensor | None = None  # torch CPU RNG state
    rng_state_cuda: torch.Tensor | None = None  # torch CUDA RNG state (if on GPU)
                ...
                # case 2: if buffer is smaller than chunk_size
                    ...
                    if finished.all():
                        if text_finished:
                            ...
                            # not the last text chunk, need to wait
                            # for next text chunk to fill up buffer,
                            # then this call ends
                            break
...
def get_rotary_cos_sin(
    ...
    inv_freq_cache: dict[tuple, torch.Tensor] | None = None,
...
def drop_tokens_from_cache(
    cache: DynamicCache | tuple | None,
    length: int,
    preserve: int,
    position_offset: int,
    rope_theta: float = 10000.0,
    inv_freq_cache: dict[tuple, torch.Tensor] | None = None,
) -> tuple[DynamicCache | None, int, bool]:
...
class StreamDecoder:
    ...
    def reset(self):
        ...
        _old_unit_count = len(self._unit_history) if hasattr(self, "_unit_history") else 0
        self._unit_history = []
    ...
    def _drop_tokens_from_cache(self, length: int) -> bool:
        ...
        _cache_type = "DynamicCache" if isinstance(self.cache, DynamicCache) else "TupleCache"
        _cache_len_before = self.get_cache_length()
        _offset_before = self._position_offset
    ...
    def _rebuild_cache_with_previous(
        ...
        units_to_keep_len: int | None = None,
    ) -> bool:
        ...
        _prev_text_preview = self._previous_text[:50] + "..." if len(self._previous_text) > 50 else self._previous_text
        _suffix_preview = self.tokenizer.decode(self._suffix_token_ids) if self._suffix_token_ids else ""
        return True

    def _slice_cache(self, start: int, end: int | None, clone: bool = True):
        """slice cache
    ...
    def _drop_unit_with_context(
        ...
        _cache_before = self.get_cache_length()
    ...
    def set_window_enabled(self, enabled: bool) -> None:
        ...
        _old_enabled = self._window_enabled
    ...
    def decode(
        ...
        Args:
            ...
            text_repetition_penalty: repetition penalty coefficient,
                >1.0 means decrease repetition,
                <1.0 means increase repetition
...
def normalize_content_item(item) -> str | Any | list[Any]:
    ...
    if isinstance(item, dict):
        ...
        if item_type == "text":
            ...
            video_frames, audio_segments, stacked_frames = get_video_frame_audio_segments(
                video_path, stack_frames=stack_frames, use_ffmpeg=use_ffmpeg, use_audio=use_audio
            )
